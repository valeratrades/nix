...
if [ "$save" -eq 0 ]; then
  ...
  alacritty -e nvim "$startdir" \
    ...
    -c "lua vim.api.nvim_create_autocmd('FileType', { pattern = 'oil', callback = function(ev) local oil = require('oil'); vim.keymap.set('n', 's', function() local entry = oil.get_cursor_entry(); if entry and entry.type == 'file' then local path = oil.get_current_dir() .. entry.name; local lnum = vim.fn.line('.'); if _G.filechooser_marks[path] then _G.filechooser_marks[path] = nil; vim.api.nvim_buf_clear_namespace(ev.buf, _G.filechooser_ns, lnum-1, lnum); else _G.filechooser_marks[path] = true; vim.api.nvim_buf_set_extmark(ev.buf, _G.filechooser_ns, lnum-1, 0, { sign_text = '*', sign_hl_group = 'DiagnosticSignWarn' }); end; end, { buffer = true }); vim.keymap.set('n', '<CR>', function() local paths = {}; for p, _ in pairs(_G.filechooser_marks) do table.insert(paths, p); end; if #paths == 0 then local entry = oil.get_cursor_entry(); if entry and entry.type == 'file' then paths = { oil.get_current_dir() .. entry.name }; end; end; if #paths > 0 then local f = io.open('$tmpfile', 'w'); if f then for i, p in ipairs(paths) do f:write(p); if i < #paths then f:write('\\n'); end; end; f:close(); end; vim.cmd('qa!'); end; end, { buffer = true }); vim.keymap.set('n', 'S', function() oil.open(vim.fn.expand('~/tmp/Screenshots/')) end, { buffer = true }); end })"
