...
-- Always update symlink if result exists (handles manual rebuilds)
if vim.fn.filereadable(result_so_path) == 1 then
	local is_symlink = vim.fn.getftype(lua_so_path) == 'link'
	local exists = vim.fn.filereadable(lua_so_path) == 1

	if not is_symlink or not exists then
...
if not ok then
	...
	-- Load the plugin
	rust_plugins = require('rust_plugins')
else
