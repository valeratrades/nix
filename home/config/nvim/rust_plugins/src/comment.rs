...
pub fn remove_end_of_line_comment() {
	...
	let save_cursor = api::get_current_win().get_cursor().unwrap_or((1, 0));
...
pub fn debug_comment(action: &str) {
	...
	match action {
		"add" => {
			...
			let cursor_position = api::get_current_win().get_cursor().unwrap_or((1, 0));
...
pub fn add_todo_comment(n: i64) {
	...
	let todo_line = format!("O{cs_str}TODO{exclamations}: ");

	api::feedkeys(&NvimString::from(todo_line), &NvimString::from("n"), false);
}
...
pub fn toggle_comments_visibility() {
	...
	let on: i64 = api::get_var("_rust_toggle_comments_on").unwrap_or(1);
