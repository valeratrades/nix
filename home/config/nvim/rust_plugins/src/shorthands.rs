...
pub fn f(s: String, mode: Option<String>) {
	...
	api::feedkeys(&NvimString::from(s), &NvimString::from(mode_str), false);
}
...
pub fn infer_comment_string() -> String {
	...
	let extension: String = api::call_function("expand", ("%:e",)).unwrap_or_else(|_| String::new());
	...
	let commentstring: String = api::get_option_value("commentstring", &Default::default()).unwrap_or_else(|_| "//".to_string());
...
fn normalize_key(key: &str) -> String {
	key.to_lowercase().replace("<c-", "<C-").replace("<m-", "<M-").replace("<a-", "<A-")
}
...
fn vim_defaults() -> HashMap<&'static str, Vec<&'static str>> {
	...
	map.insert(
		...
		vec![
			// Movement
			"h", "j", "k", "l", "gj", "gk", "H", "M", "L", "w", "W", "e", "E", "b", "B", "ge", "gE", "%", "0", "^", "$", "g_", "gg", "G", "gd", "gD", "f", "F", "t", "T", ";", ",", "}", "{",
			"(", ")", "[", "]", "zz", "zt", "zb", // Scrolling
			"<C-e>", "<C-y>", "<C-b>", "<C-f>", "<C-d>", "<C-u>", // Editing
			"r", "R", "J", "gJ", "g~", "gu", "gU", "s", "S", "u", "U", "<C-r>", ".", // Marks/Jumps
			"m", "`", "'", "<C-i>", "<C-o>", "<C-]>", "g,", "g;", // Operators
			"y", "d", "c", "p", "P", "gp", "gP", "x", "X", "Y", "D", "C", // Indent
			">", "<", "=", // Search
			"/", "?", "n", "N", "#", "*", "g*", "g#", // Tabs/Windows
			"gt", "gT", "<C-w>", // Text entry
			"a", "i", "o", "O", "I", "A", // Visual
			"v", "V", "<C-v>", // Folds
			"za", "zo", "zc", "zr", "zm", "zi", "zf", "zd", // Other
	...
	map.insert(
		...
		vec![
			// Movement
			"h", "j", "k", "l", "w", "W", "e", "E", "b", "B", "0", "^", "$", "gg", "G", "f", "F", "t", "T", ";", ",", "}", "{", "(", ")", "<C-d>", "<C-u>", "<C-f>", "<C-b>",
			// Visual
			"v", "V", "<C-v>", "o", "O", // Text objects
			"aw", "ab", "aB", "at", "ib", "iB", "it", "a", "i", // Indent
			">", "<", // Operators
			"y", "d", "c", "~", "u", "U", "r", "s", "x", "J", "gJ", "p", "P", // Other
			":", "n", "N", "*", "#",
		],
	);
	...
	map.insert(
		"x",
		vec![
			"h", "j", "k", "l", "w", "W", "e", "E", "b", "B", "0", "^", "$", "gg", "G", "f", "F", "t", "T", ";", ",", "}", "{", "(", ")", "<C-d>", "<C-u>", "<C-f>", "<C-b>", "v", "V",
			"<C-v>", "o", "O", "aw", "ab", "aB", "at", "ib", "iB", "it", "a", "i", ">", "<", "y", "d", "c", "~", "u", "U", "r", "s", "x", "J", "gJ", "p", "P", ":", "n", "N", "*", "#",
		],
	);

	// Select mode
	map.insert("s", vec!["<C-g>", "c", "C", "d", "D", "y", "Y", "x", "X"]);

	// Operator-pending mode
	map.insert(
		"o",
		vec![
			"h", "j", "k", "l", "w", "W", "e", "E", "b", "B", "f", "F", "t", "T", ";", ",", "0", "^", "$", "gg", "G", "}", "{", "(", ")", "[", "]", "a", "i", "v", "V", "<C-v>",
		],
	);
	...
	map.insert(
		"i",
		vec![
			"<C-h>", "<C-w>", "<C-j>", "<C-t>", "<C-d>", "<C-n>", "<C-p>", "<C-r>", "<C-o>", "<C-a>", "<C-x>", "<C-e>", "<C-y>", "<Esc>", "<C-c>", "<C-[>",
		],
	);
	...
	// Command mode
	map.insert("c", vec!["<C-b>", "<C-e>", "<C-f>", "<C-h>", "<C-w>", "<C-u>", "<C-n>", "<C-p>", "<C-r>"]);
...
pub fn smart_keymap(mode: Object, lhs: nvim_oxi::String, rhs: Object, opts: Object) {
	...
	let caller_info: String = opts_dict
		.get("_caller")
		.and_then(|o| {
	...
	if opts_dict.get("desc").is_none() {
		api::err_writeln(&format!("[{}] Keymap '{}' missing desc (required for which-key)", caller_info, lhs_str));
	}

	// Parse mode - handle both string and array, store as String for later use
	let obj = mode.clone();
	let mod_strings: Vec<String> = {
		// Try as string first
		...
		if let Ok(s) = s_result {
			...
			let result: Array = api::call_function("luaeval", (lua_check, obj)).unwrap_or_else(|_| Array::from_iter(vec![Object::from("n")]));
	...
	let overwrite: Option<bool> = opts_dict.get("overwrite").map(|o| {
	...
	if let Some(overwrite_value) = overwrite {
		...
		for mode_name in &expanded_modes {
			...
			let maparg_result: Dictionary = api::call_function("maparg", (lhs.clone(), &mode_name[..], false, true)).unwrap_or_else(|_| Dictionary::new());
			...
			let is_vim_default = defaults_for_mode.iter().any(|key| normalize_key(key) == normalized_lhs);

			if is_user_mapped || is_vim_default {
				found_existing = true;
				log_keymap(&format!(
					"Found existing mapping for '{}' in mode '{}': is_user_mapped={}, is_vim_default={}, overwrite={}",
		...
		if overwrite_value && !found_existing {
			let mod_str = expanded_modes.join(",");
			api::err_writeln(&format!("[{caller_info}] Unnecessary overwrite=true: '{lhs_str}' (mode '{mod_str}') has no existing mapping"));
		}
	}
