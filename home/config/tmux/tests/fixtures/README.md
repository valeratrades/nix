...
The test in `claude-sessions.rs` (`mod tests`) feeds every one through the pure
...
env RUSTC_WRAPPER="" PATH="$RUST_PATH/bin:$PATH" INSTA_UPDATE=always cargo -Zscript test --manifest-path ../../claude-sessions.rs
env RUSTC_WRAPPER="" PATH="$RUST_PATH/bin:$PATH" INSTA_UPDATE=no     cargo -Zscript test --manifest-path ../../claude-sessions.rs
```
...
```fish
set RUST_PATH (cat ~/.cargo/target/.rust-store-path)
env RUSTC_WRAPPER="" PATH="$RUST_PATH/bin:$PATH" cargo -Zscript test --manifest-path ../../claude-sessions.rs
```
