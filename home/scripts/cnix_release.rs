...
fn main() {
	...
	let sed_deps_path = format!("{}/.github/workflows/pre_ci_sed_deps.rs", std::env::var("HOME").unwrap_or_default().replace("~", "") + "/s/g/github");
	let sed_deps_path = sed_deps_path.replace("//", "/");
	let home = std::env::var("HOME").unwrap_or_default();
	let sed_deps = format!("{}/s/g/github/.github/workflows/pre_ci_sed_deps.rs", home);
	if !run(&sed_deps, &["."]) {
