...
fn sanitize_filename(filename: &str) -> String {
    ...
    let patterns_to_remove = ["SpotiDown.App - "];
...
fn main() {
    ...
    let using_flag = args.paper
        ...
        || args.wine
        || args.images
        || args.screenshot_trading
        || args.screenshot_images;
    ...
    let (from, mut to_dir) = if args.paper {
        ...
        (
            home.join("Downloads"),
            home.join("Documents/TwitterThreads"),
        )
    } else if args.wine {
        (
            home.join("Downloads"),
            home.join(".wine/drive_c/users/v/Downloads"),
        )
    } else if args.images {
    ...
    match latest_file {
        Some(from_path) => {
            ...
            let final_destination = match explicit_filename.or(extracted_filename) {
                ...
                None => {
                    if let Some(original_filename) = from_path.file_name() {
                        let sanitized_name =
                            sanitize_filename(&original_filename.to_string_lossy());
            ...
            if status.success() {
                ...
                eprintln!(
                    "Error moving file. mv command failed with status: {}",
                    status
                );
                std::process::exit(1);
            }
