...
fn parse_txt(input: &Path, chapter_re: &Regex, outdir: &Path) -> Result<u32> {
	...
	let flush =
		|num: u32, title: Option<&str>, lines: &[String], outdir: &Path| -> Result<()> {
			...
			Ok(Event::End(e)) => {
				...
				if name.as_ref() == b"body" {
					...
					if section_depth == 1 {
						if let Some(num) = current_num {
							let refs: Vec<&str> =
								paragraphs.iter().map(|s| s.as_str()).collect();
					...
					if current_num.is_none() {
						if let Some(m) = num_re.find(&title_text) {
							current_num =
								Some(title_text[m.start()..m.end()].parse().unwrap());
						}
					}
...
async fn load_page(
	...
	if !output.status.success() {
		...
		return Err(eyre!("book_parser failed for page {page}: {}", stderr.trim()));
	}
...
async fn run_book_parser(section: &Path, num: u32, de_dir: &Path, fail_dir: &Path) -> Result<()> {
...
async fn main() -> Result<()> {
	...
	match cli.cmd {
		Cmd::FromFile { file, pattern } => {
			...
			let count = match ext {
				"txt" => {
					let pat = pattern.chapter_pattern.as_deref().unwrap_or(r"^Глава [0-9]+");
