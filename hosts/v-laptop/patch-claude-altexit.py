...
the binary: overwrite the body of the stash callback (in the chat-input component) with
`<onSubmit>.current?.(<input>.current);process.exit(0)` — both refs live in the same
closure as the `chat:submit` handler, which is literally `()=>{Nt.current?.(Rt.current)}`.
The user then rebinds `chat:stash` to `alt+a` via keybindings.json.
...
# The stash callback, verbatim from claude-code 2.1.220. Prefix/suffix (the
# `useCallback` wrapper and its deps array) are preserved; only the inner `{...}` body
# is replaced. Must occur exactly once.
ANCHOR = b'PMe=Di.useCallback(()=>{if(te.trim()===""&&A!==void 0){if(At(A.text),Qe(A.cursorOffset),H(A.pastedContents),A.launchWarning)eKe(A.launchWarning);b(void 0),be("input_stash")}else if(te.trim()!=="")b({text:te,cursorOffset:He,pastedContents:R,launchWarning:pcd()??void 0}),At(""),Qe(0),H({}),hr((Wt)=>{if(Wt.hasUsedStash)return Wt;return{...Wt,hasUsedStash:!0}}),be("input_stash")},[te,He,A,At,b,R,H])'

PREFIX = b'PMe=Di.useCallback(()=>{'
SUFFIX = b'},[te,He,A,At,b,R,H])'
...
NEW_BODY = b'Nt.current?.(Rt.current);setTimeout(()=>process.exit(0),800)'
...
def die(msg: str) -> None:
    sys.stderr.write(
        ...
        "         grep -ao '.\\{0,400\\}hasUsedStash)return.\\{0,80\\}' <claude-binary>\n"
        "       and the submit closure (gives the onSubmit/input ref names):\n"
        "         grep -ao 'action:\"chat:submit\",context:\"Chat\",handler:()=>{[^}]*}' <claude-binary>\n"
